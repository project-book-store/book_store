...
export class BookManagerComponent implements OnInit {
  ...
  delete() {
    for (let i = 0; i < this.deleteList.length; i++) {
      this.bookService.deleteBook(this.deleteList[i].id).subscribe(() => {
  ...
  checkAll(event: any) {
    ...
    if (this.checkedAll) {
      ...
      this.deleteList = this.deleteList.filter(item => !this.bookList.some(item2 => item.id === item2.id));
    }
    this.checkDelete = this.deleteList.length < 1;
  }
  ...
  isCheckedAll() {
    ...
    this.checkedAll = (lengthDeleted === this.bookList.length);
  }
